...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path(__file__).resolve().parent / "runs" / "local")
    args = parser.parse_args()
    if not args.export_only and args.output.resolve() == (Path(__file__).resolve().parent / "behavioral_bias_run").resolve():
        parser.error("The published behavioral_bias_run directory is read-only; choose --output runs/NAME.")
